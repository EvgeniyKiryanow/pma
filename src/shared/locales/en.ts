import { foundationEn } from './foundation.en';

export const en = {
    ...foundationEn,
    header: {
        title: 'Personnel Manager',
        addUser: 'Add new Person',
        logout: 'Log out',
        upcomingBirthday: 'upcoming birthday',
        upcomingBirthdays: 'upcoming birthdays',
        viewBirthdays: 'View upcoming birthdays',
        birthdayTitle: '🎂 Upcoming Birthdays',
        managerTab: 'Manager',
        backupTab: 'Backups',
        remindersTab: 'Reminder',
        reportsTab: 'Reports',
        tablesTab: 'Tables',
    },
    tables: {},
    reports: {
        uploadTab: 'Templates',
        savedTab: 'Saved Reports',
        uploadTitle: 'Upload templates',
        uploadTemplate: 'Upload .docx file',
        availableTemplates: 'Available Templates',
        previewNote: 'Click to preview this template',
        previewTitle: 'Template Preview',
        saveTemplate: 'Save Template',
        selectUser: 'Select User',
        savedTemplates: 'Saved Templates',
        previewError: 'Could not preview document',
        savedSuccessfully: 'Template saved successfully!',
        noSavedTemplates: 'No saved templates',
        generateFilledTemplate: 'Generate report',
        download: 'Download report',
        fieldSelection: 'Field Selection',
        yourSavedReportsTitle: 'Your Saved Reports',
        yourSavedReportsEmpty: "You don't have any saved reports yet",
        searchTemplates: 'Search templates…',
        templatesTitle: 'Report templates',
        uploadDescription:
            'Word documents (.docx) with placeholders such as {fn_n}, {rank_n}, {awards_list} — the program fills them with the data of a person.',
        uploadTemplates: 'Choose .docx files or drop them here',
        uploadHint: 'Several at once',
        uploading: 'Saving…',
        inBackups:
            'Templates are stored in the data folder of the program and travel with backups.',
        templatesAdded: 'Templates added: {{count}}',
        onlyDocx: 'Only .docx files were added — others were skipped',
        removeTemplate: 'Delete the template',
        removeTemplateConfirm: 'The template «{{name}}» will be deleted.',
        downloadTemplate: 'Download the template (.docx)',
        preview: 'Preview',
        noTemplatesFound: 'Nothing found',
        templateStep: 'Document template',
        chooseTemplate: 'Choose a template',
        resultStep: 'The report',
        chooseBoth: 'Choose a person and a template',
        secondPerson: 'Second person (reports about two people)',
        noSecondPerson: '— none —',
        unitInfo: 'Unit details',
        downloadDocx: 'Download DOCX',
        savePdf: 'Save as PDF',
        print: 'Print',
        keep: 'Keep in «Saved reports»',
        keptInSaved: 'The report is kept in «Saved reports» and travels with backups',
        generated: 'The report is ready — check it and save',
        generatedNoPreview:
            'The report is ready, but the preview is not available. It can be downloaded.',
        previewNoteResult: 'Preview (the layout may differ slightly from Word)',
        resultHint: 'The preview of the document appears here once it is generated.',
        personStep: 'Service member',
        searchPeople: 'Search: name, rank, position…',
        filterUnit: 'Unit',
        allUnits: 'All units',
        filterStatus: 'Status',
        allStatuses: 'All statuses',
        peopleShown: '{{shown}} of {{total}} shown',
        templateFilter: {
            all: 'All',
            uploaded: 'Mine',
            bundled: 'Built-in',
        },
        templateSource: {
            uploaded: 'My template',
            bundled: 'Built-in',
        },
    },
    leftBar: {
        title: 'Personnel List',
        total: 'Total Personnel',
        searchPlaceholder: 'Search by name, rank, or ID...',
        noUsersFound: 'No users found',
    },
    reminders: {
        title: 'Reminders',
        addLabel: 'Add a new reminder',
        placeholder: 'What do you want to be reminded about?',
        addButton: 'Add Reminder',
        done: 'Done',
        undone: 'Undone',
        delete: 'Delete',
    },

    rightBar: {
        selectPrompt: 'Select a user to see details',
        edit: 'Edit',
        comments: 'Comments',
        delete: 'Delete',
        confirmDelete: 'Are you sure you want to delete',
        basicInfo: 'Basic Info',
        dateOfBirth: 'Date of Birth',
        phone: 'Phone',
        email: 'Email',
        rights: 'Rights',
        education: 'Education',
        awards: 'Awards',
        conscriptionInfo: 'Conscription Info',
        notes: 'Notes',
        relatives: 'Relatives',
        history: 'History',
        showMore: 'Show all',
        showLess: 'Show less',
    },
    backupPanel: {
        title: 'Backup Panel',
        actions: 'Backup & Restore',
        settings: 'Auto Backup Settings',
        instructions:
            'Use the options below to download a backup or restore from one. Restoring a backup will overwrite current data.',
        download: 'Download Backup',
        restore: 'Restore Backup',
        backupSuccess: '✅ Backup saved!',
        backupFail: '❌ Backup failed!',
        restoreSuccess: '✅ Restored! Please restart.',
        restoreFail: '❌ Restore failed.',
    },
    backupControls: {
        title: 'Backup Controls',
        frequency: 'Auto-backup frequency',
        everyXDays: 'Every {{days}} day{{suffix}}',
        save: 'Save',
        saving: 'Saving...',
        saveSuccess: '✔ Auto-backup every {{days}} day{{suffix}} saved.',
        saveFail: '❌ Failed to save interval.',
        location: 'Auto-backup location',
        __lang: 'en',
    },
    titleBar: {
        title: 'Control Panel Manager',
        restore: 'Restore Backup',
        reset: 'Reset App',
        reload: 'Reload',
        hide: 'Minimize',
        fullscreen: 'Full screen',
        close: 'Close',
        updateCheck: 'Check for updates',
        updateCurrent: 'You have the newest version ({{version}})',
        updateAvailableTitle: 'Version {{version}} is available',
        updateAvailableText:
            'PManager will download the update, close and start again in the new version. Save open forms first.',
        updateInstall: 'Update',
        updateDownloading: 'Downloading version {{version}}… Do not turn off the computer',
        updateManual:
            'Version {{version}} is available. Download the installer from the releases page and install it over this version — your data stays.',
        updateRestarting: 'Update downloaded. PManager is restarting…',
        restoreNoUser: 'You must have a registered user to restore a backup.',
        restoreFail: '✅ Restored! Please restart.',
        restoreSuccess: 'Restore failed. Try again.',
        resetConfirm:
            'Are you sure you want to reset the app?\nThis will delete all users and data!',
        resetSuccess: 'App has been reset. Restarting...',
        resetFail: 'Failed to reset database.',
    },
    history: {
        title: 'User History',
        add: 'Add History',
        searchPlaceholder: 'Search by description, author, date, or file...',
        noRecords: 'No history records found.',
        confirmDelete: 'Are you sure you want to delete this history entry?',
        saveError: 'Error saving history. Please try again.',
        filters: {
            '1day': '1 Day',
            '7days': '7 Days',
            '14days': '14 Days',
            '30days': '30 Days',
            all: 'All Time',
        },
    },
    historyItem: {
        delete: 'Delete entry',
        pdf: 'PDF',
        download: 'Download {{name}}',
        type: {
            history: 'History',
            comment: 'Comment',
            text: 'Text',
        },
    },
    historyModal: {
        title: 'Add History Entry',
        close: 'Close',
        description: 'Description',
        descriptionPlaceholder: 'Enter detailed description for this history entry...',
        attach: 'Attach Files',
        remove: 'Remove',
        save: 'Save History Entry',
    },
    comments: {
        title: 'User Comments',
        close: 'Close',
        authorPlaceholder: 'Author / Автор',
        textPlaceholder: 'Write a comment... / Напишіть коментар...',
        attachLabel: 'Attach files / Додати файли',
        selectFiles: 'Select files',
        removeFile: 'Remove file',
        add: 'Add Comment',
        search: 'Search comments by author, text or file name...',
        delete: 'Delete',
        deleteConfirm: 'Are you sure you want to delete this comment?',
        none: 'No comments available.',
        anonymous: 'Anonymous',
    },
    main: {
        loading: 'Loading...',
        madeBy: 'Made by',
        improvements:
            'If you have any suggestions or improvements — I’m happy to review your proposition!',
    },
    user: {
        fullName: 'Full Name',
        rank: 'Rank',
        position: 'Position',
        passportData: 'Passport Data',
        participantNumber: 'Participant Number (UBD)',
        identificationNumber: 'Identification Number',
        fitnessCategory: 'Fitness Category',
        unitNumber: 'Unit Number',
        hasCriminalRecord: 'Criminal Record',
        criminalRecordDetails: 'Criminal Record Details',
        isAttached: 'Attached from another unit',
        attachedFrom: 'Attached from (unit)',
        callsign: 'Callsign',
        militaryTicketInfo: 'Military Ticket Info',
        militaryServiceHistory: 'Military Service History',
        civilProfession: 'Civil Profession',
        educationDetails: 'Education Details',
        residenceAddress: 'Residence Address',
        registeredAddress: 'Registered Address',
        healthConditions: 'Health Conditions',
        maritalStatus: 'Marital Status',
        familyInfo: 'Family Info',
        religion: 'Religion',
        recruitingOffice: 'Recruiting Office',
        driverLicenses: 'Driver Licenses',
        bloodType: 'Blood Type',
        dateOfBirth: 'Date of Birth',
        rights: 'Rights',
        conscriptionInfo: 'Conscription Info',
        notes: 'Notes',
        email: 'Email',
        phoneNumber: 'Phone Number',
        education: 'Education',
        awards: 'Awards',
        relatives: 'Relatives',
        addRelative: 'Add Relative',
        cancel: 'Cancel',
        add: 'Add',
        saveChanges: 'Save Changes',
        uploadPhoto: 'Upload photo',
        save: 'Save',
        history: 'History',
        comments: 'Comments',
        fitnessCategoryOption: {
            fit: 'Fit for service',
            limited: 'Limited fitness',
        },
        addUser: 'New service member',
        editUser: 'Edit record',
        relativeName: 'Relative full name',
        relativeRelation: 'Relationship',
        relativePhone: 'Phone',
        soldierStatus: 'Service member status',
    },
    sections: {
        basic: 'Basic Information',
        military: 'Military Information',
        legal: 'Legal Data',
        health: 'Health',
        background: 'Civil Background',
        relatives: 'Relatives and Comments',
    },
};
