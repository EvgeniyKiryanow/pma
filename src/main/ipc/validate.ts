...
export function requireInt(value: unknown, field: string, { min = 1, max = Number.MAX_SAFE_INTEGER } = {}): number {
...
export function requireBuffer(value: unknown, field: string, { maxBytes = 200 * 1024 * 1024 } = {}): Buffer {
    if (value instanceof ArrayBuffer) {
        if (value.byteLength > maxBytes) throw new AppError('VALIDATION', undefined, { field, maxBytes });
        return Buffer.from(value);
    }
    if (ArrayBuffer.isView(value)) {
        const view = value as ArrayBufferView;
        if (view.byteLength > maxBytes) throw new AppError('VALIDATION', undefined, { field, maxBytes });
