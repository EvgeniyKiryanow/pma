...
const TOPICS: Topic[] = [
    ...
    },
    {
        ...
        group: 'daily',
        icon: <UserCog />,
        title: 'Позначити прикомандированого',
        steps: [
            'Відкрийте картку людини і натисніть «Редагувати»',
            'У розділі з підрозділом увімкніть «Прикомандирований» і вкажіть, звідки',
            'Натисніть «Зберегти» — у картці зʼявиться позначка «Прикомандирований»',
        ],
        tip: 'В «Альтернативному звіті» прикомандировані рахуються окремим рядком «Прикомандировані».',
        keywords: 'прикомандирований прикомандировані інша частина відряджений до нас',
    },
    {
        id: 'staff-view',
        group: 'daily',
    ...
    },
    {
        ...
        steps: [
            ...
            'Перевірте цифри і натисніть «Експорт у Excel», «Друк» або «Зберегти PDF»',
        ],
    ...
    },
    {
        ...
        group: 'reports',
        icon: <FileSpreadsheet />,
        title: 'Файл для «Імпульсу» (Impulse Toolkit)',
        steps: [
            'Відкрийте «Таблиці та Excel» → «Згенеровані таблиці» → «Імпульс»',
            'Подивіться, кому бракує даних (РНОКПП, дата народження, звання…), і доповніть картки',
            'Натисніть «Сформувати Excel» і оберіть, куди зберегти файл',
            'Відкрийте файл, перегляньте його й імпортуйте в Impulse Toolkit',
        ],
        tip: 'Імпульс перезаписує в картці те, що є у файлі, тому PManager заповнює клітинку лише тоді, коли значення розпізнано напевно. Те, що не вдалося розкласти по колонках (адреса, паспорт тощо), лежить праворуч у сірих колонках — звідти легко доповнити вручну.',
        keywords: 'імпульс impulse toolkit додаток 1 особовий склад вивантаження excel',
    },
    {
        id: 'print',
        group: 'reports',
        icon: <Printer />,
        title: 'Надрукувати звіт або зберегти його в PDF',
        steps: [
            'Відкрийте звіт: «Таблиці та Excel» → «Згенеровані таблиці»',
            '«Друк» — відкриється вікно друку Windows: оберіть принтер і натисніть «Друк»',
            '«Зберегти PDF» — оберіть, куди зберегти файл (наприклад, на флешку)',
        ],
        tip: 'Друкується саме та таблиця, що на екрані, на аркуші A4 горизонтально; широка таблиця зменшується, щоб поміститися. Нічого додатково встановлювати не треба.',
        keywords: 'друк надрукувати принтер pdf пдф зберегти аркуш папір',
    },
    {
        id: 'import',
        group: 'reports',
    ...
    },
    {
        ...
        tip: 'Без пароля копію не відкриє ніхто — навіть ви. Робіть копію хоча б раз на тиждень: коли копії давно не було, угорі зʼявляється жовте нагадування. Як часто нагадувати — там само, у «Повна копія».',
        keywords: 'копія резервна флешка зберегти бекап нагадування',
    },
    {
        id: 'transfer',
        group: 'data',
        ...
        steps: [
            ...
            'На новому встановіть PManager і на екрані входу відкрийте вкладку «Відновлення з копії»',
            'Оберіть файл (або просто двічі клацніть файл .pmb на флешці — програма відкриється з ним), введіть пароль, натисніть «Перевірити копію», потім «Відновити дані»',
            'Вкажіть логін і пароль адміністратора, який входитиме після відновлення, і увійдіть ними (облікові записи з копії теж працюють)',
        ],
    ...
    },
    {
        ...
        steps: [
            ...
            'Програма перезапуститься: відкрийте вкладку «Відновлення з копії», оберіть файл, введіть пароль і «Відновити дані»',
    ...
    },
    {
        ...
        steps: [
            ...
            'Ніхто не памʼятає ні пароля, ні коду — вкладка «Відновлення з копії»: потрібен лише пароль резервної копії (.pmb), після відновлення ви задаєте новий логін і пароль',
            'Немає й копії — вкладка «Новий обліковий запис»: програма почне з порожньої бази, а старі дані разом із ключем відкладе в папку резервних копій (нічого не видаляється)',
        ],
        keywords: 'пароль забув код відновлення вхід новий обліковий запис усе забув',
    },
    {
        id: 'users',
        group: 'data',
    ...
    },
    {
        ...
        group: 'data',
        icon: <LifeBuoy />,
        title: 'Щось не працює: що надіслати розробнику',
        steps: [
            'Натисніть своє імʼя внизу ліворуч → «Про програму» (або кнопку «Про програму і журнал для підтримки» тут, у «Довідці»): там версія програми, системи і тека з даними',
            'Натисніть «Зберегти журнал для підтримки» і оберіть, куди зберегти файл',
            'Передайте розробнику цей файл і опишіть, що робили, коли сталася помилка',
        ],
        tip: 'Журнал — це технічні записи роботи програми (що запускалось і які були помилки); картки людей туди не записуються. Передавайте його так само обережно, як інші службові файли.',
        keywords: 'помилка підтримка журнал лог версія про програму розробник',
    },
    {
        id: 'wipe',
        group: 'data',
...
const CONVENIENCES = [
    ...
    'Кнопка поруч із хрестиком розгортає вікно на весь екран або зменшує його; розмір і місце вікна програма запамʼятовує',
