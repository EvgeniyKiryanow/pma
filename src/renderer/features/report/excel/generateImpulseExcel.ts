...
const NOTES = [
    ...
    [
        'Що заповнено',
        'Колонку заповнено полями особової картки PManager (як в Імпульсі), а для старших карток — лише тим, що розпізнано напевно: дата, серія і номер документа, значення з довідника Імпульсу (звання, стать, група крові, сімейний стан, придатність, вид служби). Решта клітинок порожні — Імпульс не змінить ці поля в картці.',
    ],
    [
    ...
    [
        'Перед імпортом',
        'Перегляньте файл. Прізвище та імʼя обовʼязкові й використовуються для пошуку картки. «Освіта і курси» — рядки з розділу «Освіта і курси» картки; для старших карток — те, що вдалося розпізнати: «Навчальний заклад» і «Тип навчального закладу» заповніть самі — без них рядок не імпортується.',
    ],
];
...
function educationSheet(wb: ExcelJS.Workbook, people: User[]): void {
    ...
    for (const person of people) {
        for (const values of impulseEducationRows(person)) {
