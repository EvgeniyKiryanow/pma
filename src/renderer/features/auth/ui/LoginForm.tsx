...
/** The «Вхід» tab: sign-in, or the password reset with a recovery code. */
export default function LoginForm({
    ...
    if (!hasAccounts) {
        return (
            <div className="space-y-4">
                <Alert tone="info" title={t('auth.login.noAccountsTitle')}>
                    {t('auth.login.noAccounts')}
                </Alert>
                <div className="flex flex-wrap gap-2">
                    <Button onClick={() => onTab('create')}>{t('auth.tabs.create')}</Button>
                    <Button variant="secondary" onClick={() => onTab('restore')}>
                        {t('auth.tabs.restore')}
                    </Button>
                </div>
            </div>
    ...
    return (
        <form onSubmit={submit} className="space-y-4">
            ...
            <p className="border-t border-line pt-4 text-center text-[13px] leading-relaxed text-ink-3">
                {t('auth.login.forgotAll')}
            </p>
        </form>
    );
}
