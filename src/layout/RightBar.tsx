...
export default function RightBar() {
    ...
    return (
        <aside className="flex-1 bg-white px-4 sm:px-6 md:px-10 shadow-inner overflow-y-auto max-h-[calc(100vh-56px)]">
            ...
            {/* ✅ Dynamic layout: 2 columns if sidebarCollapsed = true */}
            <div
                className={`pb-10 gap-6 ${
                    sidebarCollapsed
                        ? 'grid grid-cols-1 lg:grid-cols-2 max-w-full' // 2 columns when leftbar hidden
                        : 'max-w-3xl mx-auto space-y-8' // single column when leftbar open
                }`}
            >
                {/* === COLUMN 1: USER INFO === */}
                <div className="space-y-8">
                    ...
                    {/* Extra sections only if "Show More" */}
                    {showFullInfo && (
