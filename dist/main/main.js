...
const dbPath = require$$0$1.join(require$$3.app.getPath("userData"), "users.db");
let db = null;
async function initDb() {
  await require$$3.app.whenReady();
...
function getDb() {
  if (!db) {
    throw new Error("Database is not initialized yet. Call initDb() first.");
  }
  return db;
}
async function registerDbHandlers() {
  await initDb();
  require$$3.ipcMain.handle("get-users", () => {
    const db2 = getDb();
    const rows = db2.prepare("SELECT * FROM users").all();
    return rows.map((u) => ({
      ...u,
  ...
  require$$3.ipcMain.handle("add-user", (_event, user) => {
    const db2 = getDb();
    const stmt = db2.prepare(`
      INSERT INTO users (
        id, fullName, email, phoneNumber, dateOfBirth,
        position, rank, rights, conscriptionInfo, notes, photo,
    ...
    stmt.run({
      ...user,
      relatives: JSON.stringify(user.relatives),
      comments: JSON.stringify(user.comments),
      history: JSON.stringify(user.history)
    });
    return { success: true };
  });
  require$$3.ipcMain.handle("delete-user", (_event, id) => {
    const db2 = getDb();
    const stmt = db2.prepare("DELETE FROM users WHERE id = ?");
    stmt.run(id);
    return { success: true };
  });
  require$$3.ipcMain.handle("download-db", async () => {
    try {
  ...
  require$$3.ipcMain.handle("add-comment", (_event, userId, comment) => {
    try {
      const db2 = getDb();
      const user = db2.prepare("SELECT comments FROM users WHERE id = ?").get(userId);
      if (!user) return { success: false, error: "User not found" };
      const comments = JSON.parse(user.comments || "[]");
      comments.push(comment);
      const stmt = db2.prepare("UPDATE users SET comments = ? WHERE id = ?");
      stmt.run(JSON.stringify(comments), userId);
  ...
  require$$3.ipcMain.handle("delete-comment", (_event, userId, commentId) => {
    try {
      const db2 = getDb();
      const user = db2.prepare("SELECT comments FROM users WHERE id = ?").get(userId);
      if (!user) return { success: false, error: "User not found" };
      let comments = JSON.parse(user.comments || "[]");
...
console.log(require("better-sqlite3").constructor);
